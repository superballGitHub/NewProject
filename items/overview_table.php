...
class quiz_overview_table extends quiz_attempts_report_table {
    ...
    protected function add_average_row($label, $users) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(quiza.sumgrades) AS grade, COUNT(quiza.sumgrades) AS numaveraged
                  FROM $from
                 WHERE $where", $params);
				// echo "grade : $record->grade";
				// echo "numaveraged : $record->numaveraged";
      //  $record->grade = quiz_rescale_grade($record->grade, $this->quiz, false);
		$record->grade = format_float($record->grade * 100 /  $this->quiz->sumgrades, 0);
		//echo "$record->grade";
        ...
        $averagerow = array(
            $namekey    => $label,
            'sumgrades' => $this->format_average_percent($record),
     ...
     */
    protected function format_average_percent($record, $question = false) {
        if (is_null($record->grade)) {
			...
			$average = format_float( $record->grade, 0);
			//$average = format_float($record->grade * 100 /  $record->numaveraged, 0);
		    //$average= $average.'%';
        }
    ...
    public function col_sumgrades($attempt) {
		...
		$grade = format_float($attempt->sumgrades * 100 /  $this->quiz->sumgrades, 0);
		$grade= $grade.'%';
        if ($this->is_downloading()) {
            return $grade;
        }
